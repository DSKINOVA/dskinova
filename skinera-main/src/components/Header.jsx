...
useEffect(() => {
    ...
    // Since the top bar is inside the component, we can check a fixed value,
    // but a reliable way is to check if the window has scrolled.
    const handleScroll = () => {
      // Use 1 to make it sticky as soon as scrolling starts
      const offset = window.scrollY; 
      
      // You might want a higher value if you want the top bar to disappear first
      // const topBarHeight = 40; // Approx. height in pixels of the top bar
      // const shouldBeSticky = offset > topBarHeight;
      
      setIsSticky(offset > 1);
    };
  ...
  return (
    <header 
      ref={headerRef}
      className={`bg-white text-black z-50 transition-all duration-300 ${
        // Apply fixed classes when isSticky is true
        isSticky
          ? "fixed top-0 left-0 right-0 shadow-lg border-none" 
          : "relative border-b border-black/10"
      }`}    >
      {/* Top contact bar */}
      <div 
        className={`bg-[#c67c54] text-white text-xs sm:text-sm border-b border-black/10 font-bold transition-all duration-300 overflow-hidden ${
          isSticky ? 'max-h-0 py-0 opacity-0' : 'max-h-10 opacity-100'
        }`}
      >
